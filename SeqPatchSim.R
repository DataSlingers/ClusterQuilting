...
## Create simulated low-rank Gaussian mixture model data, along with sequential patch observed missingness.
## Inputs:
...
# iters: number of iterations to repeat simulation (for creating simulation study)
# save_dir: overall save folder, which will contain subfolders of individual sim iterations
seq_create <- function(n, p, n_per, b, clusts, rank_sim, mean_cons, sd_cons,
  ...
  for(iter in 1:iters){
    print(iter)
    dir_name <-  paste0("./clusts_", clusts, "_n_", n, "_k_", b, "_views_", 
                        v,  "_vpb_", h, "_means_", mean_cons, "_rank_", rank_sim, "_off_", sd_mult,
                        "_", iter)
    print(dir_name)
    dir.create(dir_name)
    setwd(dir_name)
    ...
    # Save information directly for estimation downstream.
    ...
    ### Save true clustering assignments
    ...
    ## view_features: same as patch_features, kept here for parallel structure with mosaic sims
    ## patch_obs: observations within each patch
    ## patch_features: features within each patch
    ## view_obs:  same as patch_obs, kept here for parallel structure with mosaic sims
    ...
    save_info[[iter]] <- list(masked_dat = t(masked_dat),
                full_dat = t(data_mat),
